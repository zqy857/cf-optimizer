...
`cf_lifecycle.py` 与健康度、策略共用同一套状态口径, 三者联动而非各干各的:

- **状态标签(按排名分层)**: 每协议按**健康分排名**, 前 N(=「目标可用IP数」)名且达标新鲜 -> `active`(**高频** 30~60min 巡检); 其余存活 -> `reserve`(**低频** 6~24h); 另有 `probation`(退避)/`dead`。每个 IP 落 `state`/`interval`。所以"目标可用IP数内的 IP 高频、其余低频"正是当前行为。
- **监控**: 库满(可用数达标)后不再一轮轮扫, 改为**事件驱动**——睡到下一个 `next_check_at` 才醒; 到期时间由状态周期(active 30~60min、reserve 6~24h)加**稳定抖动**错峰, 不扎堆。
- **剔除**: 失效清退、**每前缀去重**(v4 /24、v6 /48 各留上限, 可配, 保护 pin/active)、国家均衡、库容上限; 所有剔除都写**墓碑+24h 冷却**, 不会被立刻重测。
- **补充(需求驱动)**: 算每协议缺口; 有热缺口优先从 `reserve` **提升确认**, 不足才触发探索; 发现侧邻域名单**自动排除已达配额的前缀**, 避免"补了又删"。
- **两个数就够了(热目标 + 库上限)**: 只需设「目标可用IP数」和「库上限」, **备胎目标自动 = 库上限 − 热目标**。
  - `active ≥ 热目标` → 进入维护/值守(停止大范围发现);
  - 维护期若总库 < 上限 → **低频**补备胎, 养到上限即**完全静默**; 上限=0 表示不养备胎(到热目标即停)。
  - `热目标 > 上限` 自动夹取(热目标压到 ≤ 上限), 避免"补了又裁"抖动。
