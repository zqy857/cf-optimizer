...
4. **⚙️ 系统设置**(管理控制台):
   - **服务管理**: 查看服务运行状态与开机自启状态, 一键 **重启服务** / **停止服务**, 以及一个按当前状态自动切换文案的**开机自启开关**(已开启时显示「关闭开机自启」, 否则显示「开启开机自启」)。systemd 托管部署且已由安装脚本授予 sudo 权限时全部可用; 非托管(手动 / 容器)或未授权时, 开机自启开关会被禁用, 重启 / 停止退化为进程内操作(重启 = 自重启, 停止 = 退出进程, 停止后需到设备上重新启动或重启设备恢复)。
