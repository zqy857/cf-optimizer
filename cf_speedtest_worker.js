// Cloudflare Worker 测速服务 (v2 - 修复免费版 CPU 限制)
// 用途: 替代 speed.cloudflare.com 给 CF 优选IP 工具做带宽实测
//   请求 GET /__down?bytes=N   ->  全速返回 N 字节数据
...
export default {
  async fetch(request) {
    ...
    const totalChunks = Math.min(Math.ceil(want / CHUNK_SIZE), MAX_CHUNKS);
    ...
    return new Response(stream, {
      headers: {
        ...
        "Content-Length": String(totalChunks * CHUNK_SIZE),
